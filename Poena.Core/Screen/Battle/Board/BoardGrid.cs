...
namespace Poena.Core.Screen.Battle.Board
{
    public class BoardGrid
    {
        ...
        private readonly BoardTile[,,] _grid;
        ...
        public BoardGrid(AssetManager assetManager, SpriteBatch spriteBatch, OrthographicCamera camera, BoardSize boardSize)
        {
            this._grid = new BoardGridBuilder().SetSize(boardSize).Build();
        ...
        {
            this.ForEach(bt => {
                if (bt != null && bt.IsVisible && _camera.BoundingRectangle.Intersects(bt.Position.WorldPosition))
                {
                    _spriteBatch.Draw(_assetManager.GetTexture(Assets.GetTile(bt.TileType)), bt.Position.WorldPosition.TopLeft, Color.White);

                    if (Config.DEBUG_RENDER)
                    {
                        Vector2 pos = bt.Position.GetWorldAnchorPosition();
                        _spriteBatch.DrawDebugString(bt.Position.GridSlot.ToString(), pos);
                    }
                }
            });
            ...
            if (Config.DEBUG_RENDER)
            {
                _spriteBatch.DrawRectangle(GridBounds, Color.Red, 30);
            }
        }
